...
def _compute_left_right(surface, start, end, head_length=None, head_angle_rad=None):
    ...
    start, end = map(lambda xy: scr2phy(xy, surface), (start, end))
    ...
    left = end + left_vec
    right = end + right_vec
    left, right = map(lambda xy: phy2scr(xy, surface), (left, right))
    return left, right


def _draw_body_left_right(surface, color, start, end, head_length, head_angle_rad, width):
...
def draw_arrow_by_angle_rad(
    ...
    start, end = map(lambda xy: phy2scr(xy, surface), (start, end))

    _draw_body_left_right(surface, color, start, end, head_length, head_angle_rad, width)
